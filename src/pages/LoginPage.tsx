...
export default function LoginPage() {
  ...
  return (
    <div id="login-container" className="min-h-screen bg-black text-white relative flex flex-col items-center justify-center p-6 select-none overflow-hidden font-sans selection:bg-amber-500/30 selection:text-amber-200">
      {/* CodeFuser Glowing Background Ambient Flares */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[400px] bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-amber-500/15 via-orange-600/5 to-transparent blur-3xl pointer-events-none" />
      <div className="absolute bottom-0 right-0 w-[450px] h-[450px] bg-[radial-gradient(circle_at_bottom_right,_var(--tw-gradient-stops))] from-amber-600/10 via-transparent to-transparent blur-2xl pointer-events-none" />
      
      {/* Subtle Micro-Grid Overlay */}
      <div 
        className="absolute inset-0 bg-[radial-gradient(#27272a_1px,transparent_1px)] [background-size:24px_24px] opacity-25 pointer-events-none" 
      />
      ...
      <div className="absolute top-6 left-6 sm:left-12 z-20">
        <button
          onClick={() => navigate("/")}
          className="flex items-center gap-2 text-xs font-mono font-bold uppercase tracking-widest text-zinc-400 hover:text-white transition-all cursor-pointer bg-zinc-900/60 hover:bg-zinc-800/80 backdrop-blur-md px-4 py-2.5 rounded-full border border-zinc-800/80 shadow-lg hover:border-amber-500/40 group"
        >
          <ChevronLeft size={14} className="text-amber-500 group-hover:-translate-x-0.5 transition-transform" />
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        >
          <motion.div 
            ...
            className="inline-flex items-center gap-2 px-3.5 py-1.5 bg-gradient-to-r from-amber-500/10 via-orange-500/15 to-amber-500/10 border border-amber-500/30 rounded-full text-amber-400 text-[10px] font-mono uppercase tracking-widest mb-4 shadow-[0_0_25px_rgba(245,158,11,0.2)]"
          >
        ...
        <motion.div 
          ...
          className="bg-zinc-950/85 backdrop-blur-2xl border border-zinc-800/90 hover:border-amber-500/30 p-8 rounded-3xl shadow-[0_35px_80px_rgba(0,0,0,0.95)] transition-all duration-300 relative overflow-hidden"
        >
          {/* Animated Scanning Laser Border Highlight */}
          ...
          <form onSubmit={handleAuthSubmit} className="space-y-4">
            ...
            <AnimatePresence mode="wait">
              {errorMsg && (
                <motion.div 
                  ...
                  exit={{ opacity: 0, y: -8 }}
                  className="p-3.5 bg-red-500/10 border border-red-500/30 text-red-400 text-xs rounded-xl flex items-center gap-2.5 font-sans shadow-[0_0_15px_rgba(239,68,68,0.1)]"
                >
              ...
              {successMsg && (
                <motion.div 
                  ...
                  animate={{ opacity: 1, y: 0, scale: 1 }}
                  exit={{ opacity: 0, y: -8 }}
                  className="p-3.5 bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-xs rounded-xl flex items-center gap-2.5 font-sans shadow-[0_0_15px_rgba(16,185,129,0.1)]"
                >
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full bg-gradient-to-r from-amber-500 via-orange-500 to-amber-600 hover:brightness-110 text-black font-extrabold text-xs uppercase tracking-wider py-3.5 rounded-xl transition-all shadow-[0_0_25px_rgba(245,158,11,0.25)] hover:shadow-[0_0_35px_rgba(245,158,11,0.45)] flex items-center justify-center gap-2 cursor-pointer mt-6 active:scale-[0.99]"
            >
              {isLoading ? (
                <>
            ...
            <button
              ...
              disabled={isLoading}
              className="w-full bg-zinc-900/80 hover:bg-zinc-800/90 text-zinc-200 font-semibold text-xs border border-zinc-800 hover:border-zinc-700 py-3.5 rounded-xl transition-all flex items-center justify-center gap-2.5 cursor-pointer shadow-md active:scale-[0.99]"
            >
